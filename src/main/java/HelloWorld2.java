        ...
        try {
           ...
           if (args.length > 0) {
               url = new File(args[0]).toURI().toURL();
                ...
                url = HelloWorld.class.getResource("helloworld2.config.xml");
            }

            System.out.println("Loading...");

            final ConfigurationManager cm = new ConfigurationManager(url);
	    ...
	    final Microphone microphone = (Microphone) cm.lookup("microphone");
			...
			if(resultText.equalsIgnoreCase("command"))
 			...
 			else if (resultText.equalsIgnoreCase("close fire wall"))
 		    {
 		        try{
 		        Process p;	//resultText="";
 		        final String status = "status eq Windows Firewall";
