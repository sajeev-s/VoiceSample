...
public class AlignerDemo {
    ...
    public static void main(final String args[]) throws Exception {
        ...
        if (args.length > 1) {
            ...
            audioUrl = AlignerDemo.class.getResource("test.wav");//10001-90210-01803
            transcript = TEXT;
        }
        final String acousticModelPath =
                (args.length > 2) ? args[2] : ACOUSTIC_MODEL_PATH;
        final String dictionaryPath = (args.length > 3) ? args[3] : DICTIONARY_PATH;
        final String g2pPath = (args.length > 4) ? args[4] : null;
        final SpeechAligner aligner =
                new SpeechAligner(acousticModelPath, dictionaryPath, g2pPath);

        final List<WordResult> results = aligner.align(audioUrl, transcript);
        final List<String> stringResults = new ArrayList<String>();
        for (final WordResult wr : results) {
            stringResults.add(wr.getWord().getSpelling());
        }
        
        final LongTextAligner textAligner =
                new LongTextAligner(stringResults, 2);
        final List<String> sentences = aligner.getTokenizer().expand(transcript);
        final List<String> words = aligner.sentenceToWords(sentences);
        
        final int[] aid = textAligner.align(words);
        ...
        for (int i = 0; i < aid.length; ++i) {
            if (aid[i] == -1) {
                ...
                if ((aid[i] - lastId) > 1) {
                    for (final WordResult result : results.subList(lastId + 1,
                            aid[i])) {
