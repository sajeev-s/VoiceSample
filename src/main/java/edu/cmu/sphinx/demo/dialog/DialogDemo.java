...
public class DialogDemo {
    ...
    private static double parseNumber(final String[] tokens) {
    ...
    private static void recognizeDigits(final LiveSpeechRecognizer recognizer) {
        ...
        while (true) {
            final String utterance = recognizer.getResult().getHypothesis();
    ...
    private static void recognizerBankAccount(final LiveSpeechRecognizer recognizer) {
        ...
        while (true) {
            ...
            if (utterance.endsWith("back")) {
                ...
                final double deposit = parseNumber(utterance.split("\\s"));
                savings += deposit;
                System.out.format("Deposited: $%.2f\n", deposit);
            } else if (utterance.startsWith("withdraw")) {
                final double withdraw = parseNumber(utterance.split("\\s"));
                savings -= withdraw;
    ...
    private static void recognizeWeather(final LiveSpeechRecognizer recognizer) {
...
//        configuration.setSampleRate(sampleRate);
        ...
        final LiveSpeechRecognizer jsgfRecognizer =
            new LiveSpeechRecognizer(configuration);

        configuration.setGrammarName("digits.grxml");
       /* final LiveSpeechRecognizer grxmlRecognizer =
            new LiveSpeechRecognizer(configuration);*/

        configuration.setUseGrammar(false);
        configuration.setLanguageModelPath(LANGUAGE_MODEL);
       /* final LiveSpeechRecognizer lmRecognizer =
            new LiveSpeechRecognizer(configuration);*/
